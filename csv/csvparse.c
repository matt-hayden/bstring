...
int ret;
	...
	if (tape->b->slen < TAPE_SQUASH_THRESHOLD) return 0;

	/* We don't need anything before the current head, so we
	   reduce memory usage here. */
	if (0 > (ret = bdelete (tape->b, 0, tape->ofs))) return ret;
	tape->ofs = 0;
	return 0;
}
...
int c, j;
	...
	if (0 > c || !parse_isNoWSChar[c]) {
		tapeMoveHead (tape, -j);
		if (len) *len = 1; /* NoWSChar alone */
		return 1;
	}

	tapeMoveHead (tape, 1);

	if (len) *len = 2 + j; /* 2 * NoWSChar + the RawChar* sequence */
	return 1;
}
...
int c, i, j, pos, len, ret;
	...
	if (!entry || !csvs || !csvs->tape.s || !csvs->tape.b || csvs->tape.ofs < 0) return -__LINE__;
	...
	if (0 > (ret = tapeSquashPrevious (&csvs->tape))) return ret;
	i = 0;
	...
	j = 0;
	if (0 == parseNonBlankEntry (&j, &csvs->tape)) return -__LINE__;
	pos = csvs->tape.ofs - j;
	len = j;
	...
	if ('"' == csvs->outsHdr.data[0]) csvInPlaceFixQuotes (&csvs->outsHdr);
...
bstring b;
	...
	if (NULL == entry->contents) {
		if (CNM_NEXT_FIELD != entry->mode) return -__LINE__;
		os->prevMode = entry->mode;
		if (os->msdos) {
	...
	if ((entry->contents->slen > 0 && (parse_isWSChar[entry->contents->data[0]] || parse_isWSChar[entry->contents->data[entry->contents->slen-1]])) || BSTR_ERR != binchr (entry->contents, 0, &needquotes)) {
		...
		for (i=0; i < len; i++) {
			if ('"' == entry->contents->data[i]) bconchar (b, (char) '"');
			bconchar (b, entry->contents->data[i]);
		}
		bconchar (b, (char) '"');
		ret = (int) os->fw (b->data, b->slen, 1, os->parm);
		bdestroy (b);
		if (1 != ret) return -__LINE__;
	} else {
		if (os->alwaysQuote) {
			if (0 == entry->contents->slen) {
				if (1 != os->fw (bsStaticBlkParms("\"\""), 1, os->parm)) return -__LINE__;
			} else {
...
L1:;
...

	{
		...
		if (csvs) {
			...
			while (0 <= (ret = parseCSVNextEntry (&entry, csvs))) {
				if (!benchmarkMode) {
					if (NULL != ofp) csvAppend (&os, &entry);
					if (entry.contents) {
				...
				switch (entry.mode) {
					case CNM_NEXT_ENTRY: coords.col++; break;
					case CNM_NEXT_FIELD: coords.col = 0; coords.row++; break;
					case CNM_NEXT_END: if (!benchmarkMode) printf ("<EOF>\n"); break;
					default: return -__LINE__;
				}
				if (0 == ret) break;
			}
			if (0 > ret) fprintf (stderr, "Error in CSV parsing\n");
			parseCSVClose (csvs);
			if (NULL != ofp) fclose (ofp);
		}
	}
	...
	if (benchmarkMode) fprintf (stderr, "Time: %fs\n", (end-start) / (double) CLOCKS_PER_SEC);
